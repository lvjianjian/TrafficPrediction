    ...
    public HashMap<String, Object> roadsWithNum() {
        ...
        for (int i = 0; i < lists.length; i++) {
            ...
            for (int j = 0; j < list.size(); j++) {
                ...
                doubles[i].add(new double[]{vertexMap.get(edge.getStartVertex()).getLongitude(),
                        vertexMap.get(edge.getStartVertex()).getLatitude(),
                        vertexMap.get(edge.getEndVertex()).getLongitude(),
                        vertexMap.get(edge.getEndVertex()).getLatitude()});
            }
        }
