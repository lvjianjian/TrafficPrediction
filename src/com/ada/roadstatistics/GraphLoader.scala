...
class GraphLoader(val sc: SparkContext) extends Logging with Serializable{
  ...
  /**
    ...
    * @return
    */
  def loadNewEdgeFromDataSource(edgeFile: String): RDD[(Long, ((Double, Double), (Double, Double), Int))] = {
    logInfo("Loading Edge data from %s".format(edgeFile))
    sc.textFile(edgeFile).map {
      x => val temp = x.split(":")
//        if (temp.length != 10) {
...
//        }
        ...
        (id, ((startLonLat(0).toDouble, startLonLat(1).toDouble), (endLonLat(0).toDouble, endLonLat(1).toDouble), length))
    }
  }
